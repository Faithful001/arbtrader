...
class PricingService:
    ...
    async def ingest_for_region(self, db: AsyncSession, region: str) -> int:
        ...
        for card in cards:
            # 1. Fetch listings from eBay (slow network I/O) OUTSIDE of any transaction block
            try:
                ...
                logger.error("Failed to fetch listings from eBay for card", card=card.name, error=str(e))
                continue

            # 2. Write to DB in a fast transaction and commit immediately per card
            try:
                ...
                if card_ingested > 0:
                    ...
                    logger.info("Card ingested and committed successfully", card=card.name, records=card_ingested)
            except Exception as e:
                await db.rollback()
                logger.error("Ingestion failed for card - rolled back", card=card.name, error=str(e))
